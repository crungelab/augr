...
float CvToFreq(float cv) {
    return 261.6255653f * std::pow(2.f, cv);
}
...
void Dexie::CreateControls() {
    ...
    // Oscillator section
    {
        ...
        auto coarse   = CreateFloatParameter("Coarse",   ControlMeta::kDefault, &ratio_coarse_, 1.f,  0.f,  16.f,  1.f);
        auto fine     = CreateFloatParameter("Fine",     ControlMeta::kDefault, &ratio_fine_,   0.f,  0.f,  0.99f, 0.01f);
        auto detune   = CreateFloatParameter("Detune",   ControlMeta::kDefault, &detune_,       0.f, -7.f,  7.f,   1.f);
        auto level    = CreateFloatParameter("Level",    ControlMeta::kDefault, &output_level_, 1.f,  0.f,  1.f,   0.01f);
        auto feedback = CreateFloatParameter("Feedback", ControlMeta::kDefault, &feedback_,     0.f,  0.f,  1.f,   0.01f);
...
void Dexie::Process() {
    ...
    for (std::size_t i = 0; i < frames; ++i) {
        ...
        if (gate && !gate_prev_)      stage_ = Stage::kR1;
        else if (!gate && gate_prev_) stage_ = Stage::kR4;
        gate_prev_ = gate;
        ...
        switch (stage_) {
        case Stage::kIdle: break;
        case Stage::kR1:   advance(0, Stage::kR2); break;
        case Stage::kR2:   advance(1, Stage::kR3); break;
        case Stage::kR3:   advance(2, Stage::kR3); break;  // sustain
        case Stage::kR4:
