...
// ---------------------------------------------------------------------------
class FloatParameter : public ScalarParameterT<fy_real> {
public:
    using BindingPtr = ScalarParameterT<fy_real>::BindingPtr;

    FloatParameter(std::string label, ControlMeta meta, BindingPtr binding,
                   fy_real init, fy_real min, fy_real max, fy_real step)
        : ScalarParameterT<fy_real>(std::move(label), std::move(meta),
                                    std::move(binding), init, min, max, step) {}

    fy_real NormalizedInit() const { return GetNormalizedFor(init()); }
    fy_real NormalizedMin()  const { return GetNormalizedFor(min()); }
    fy_real NormalizedMax()  const { return GetNormalizedFor(max()); }
...
FloatParameter::Make(std::string label, ControlMeta meta,
                     FloatParameter::BindingPtr binding,
                     fy_real init, fy_real min, fy_real max, fy_real step) {
    switch (meta.Unit()) {
