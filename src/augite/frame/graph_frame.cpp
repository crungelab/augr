...
GraphFrame::GraphFrame(const std::string &label)
    ...
    // gets invoked on the initial document. GraphFrame owns both doc_
...
//void GraphFrame::RebuildView() { view_ = std::make_unique<SubrackView>(doc()); }

void GraphFrame::RebuildView() {
...
nlohmann::json GraphFrame::ViewToJson() {
...
void GraphFrame::ViewFromJson(const nlohmann::json &j) {
    if (!view_) return;
...
void GraphFrame::PollPendingDialog() {
...
void GraphFrame::StartOpenDialog() {
...
void GraphFrame::StartSaveAsDialog() {
...
void GraphFrame::DoNew() {
    doc().NewDocument();
    // Load hook fires from inside NewDocument and rebuilds the view.
}

void GraphFrame::DoOpen(const std::filesystem::path &p) {
    if (!doc().Load(p)) {
...
void GraphFrame::DoSave() {
    if (!doc().Path())
        return;
    DoSaveAs(*doc().Path());
}

void GraphFrame::DoSaveAs(const std::filesystem::path &p) {
    if (!doc().Save(p)) {
...
void GraphFrame::DrawUnsavedModal() {
