...
public:
    Viewer(const std::string &label, Document &doc) : Frame(label), doc_(&doc) {}
...
template <typename TDoc, typename TView, typename TController, typename TBase = Viewer>
class ViewerT : public TBase {
public:
    template <typename... Args>
    ViewerT(const std::string &label, TDoc &doc, Args&&... args) : TBase(label, doc, std::forward<Args>(args)...) {}
    // Accessors
...
public:
    ...
    virtual Viewer::Ptr Produce(const std::string &label, Document &doc, Model &model) = 0;
    virtual std::type_index GetKey() = 0;
};

template <typename T, typename N = Model>
class ViewerFactoryT : public ViewerFactory {
    Viewer::Ptr Produce(const std::string &label, Document &doc, Model &model) override {
